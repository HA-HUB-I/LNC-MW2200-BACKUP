...
class ModbusWorker(threading.Thread):
    ...
    def _poll(self):
        try:
            ...
            if r_main.isError(): raise Exception("Modbus Read Error")
            main = r_main.registers
            ...
            if self._consecutive_errors == 1:
                print(f"Machine disconnected or OFF (Waiting for reconnection...)")
            self.client.close()
    ...
    def run(self):
        while True:
            ...
            if self.client.is_socket_open():
                ...
                # Машината е изключена - чакаме повече (5 секунди)
                time.sleep(5.0)
